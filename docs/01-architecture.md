...
### VMs completas (`VmLabDriver`) — postura de segurança diferente do driver Docker

O driver de VM (`apps/api/src/labs/drivers/vm.driver.ts`) provisiona desktops completos — Windows 10 (via QEMU dentro de um container, imagem `dockurr/windows`, com ISO de avaliação oficial da Microsoft de 90 dias) e Ubuntu Desktop (`dorowu/ubuntu-desktop-lxde-vnc`) — acessíveis pelo navegador via noVNC. Ele **não aplica** `--cap-drop ALL` nem `--read-only`, porque o QEMU precisa escrever o disco virtual em `/storage` e o container Windows precisa de `--cap-add NET_ADMIN` para criar a interface de rede interna da própria VM. `--security-opt no-new-privileges` e os limites de CPU/memória/pids continuam aplicados.

Isso significa que, para VMs, a rede isolada `tica-labs-isolated` (`internal: true`) passa a ser a **única** linha de defesa contra saída para a internet ou para a rede corporativa — não há uma segunda camada de hardening no próprio container. Por isso essas VMs rodam num **host dedicado** (fora do Render), operado conforme `infra/labs/`, e a API se conecta a esse host via Docker remoto com TLS mútuo (`DOCKER_HOST`). Ver `docs/06-deployment.md`.
