...
- Para expor o console noVNC de cada VM em HTTPS (obrigatório em produção, já que `apps/web` roda em HTTPS e um iframe `http://` seria bloqueado por mixed content), usar `LAB_VM_ACCESS_MODE=traefik-labels`: um Traefik nesse host, com um domínio wildcard (`LAB_PUBLIC_DOMAIN`) e certificado Let's Encrypt automático, expõe cada instância em `https://lab-<instanceId>.<LAB_PUBLIC_DOMAIN>/`.
